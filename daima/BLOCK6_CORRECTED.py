# ==============================================================================
# BLOCK 6: 模型加载函数（直接采用training notebooks代码）
# ==============================================================================
...
def load_unet(base_dir, device):
    """UNet - 直接从7_UNet/training notebook"""
    try:
...
def load_deeplabv3(base_dir, device):
    """DeepLabV3+ - 直接从8_DeepLabV3/training notebook"""
    try:
        import segmentation_models_pytorch as smp
...
def load_sam2_tiny(base_dir, device):
    """SAM2-Tiny - 直接采用SAM2-Tiny模型训练.ipynb"""
    try:
...
def load_mobilesam(base_dir, device):
    """MobileSAM - 直接采用2_MobileSAM模型训练.ipynb"""
    try:
        import torch.nn as nn
...
def load_sam21_tiny(base_dir, device):
    """SAM2.1-Tiny - 直接采用5_SAM21_Tiny模型训练.ipynb"""
    try:
        import torch.nn as nn
